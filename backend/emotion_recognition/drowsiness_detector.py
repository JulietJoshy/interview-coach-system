...
class DrowsinessDetector:
    ...
    Uses OpenCV's Haar cascades for face and eye detection as a fallback
    when dlib is not available.
    """
    
    # EAR and MAR thresholds
    EAR_THRESHOLD = 0.25       # Below this = eyes closing
    MAR_THRESHOLD = 0.45       # Above this = yawning (lowered for better detection)
    CONSECUTIVE_FRAMES = 3     # Frames below threshold to count as blink/drowsy
    YAWN_CONSECUTIVE = 3       # Frames above MAR threshold to count as yawn (lowered for sensitivity)
    
    def __init__(self):
        """Initialize drowsiness detector with dlib or fallback to OpenCV"""
        self.use_dlib = False
        self.detector = None
        self.predictor = None
